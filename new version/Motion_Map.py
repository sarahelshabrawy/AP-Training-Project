...
class Ui_MotionMapWindow(QMainWindow):
    ...
    def setupUi(self):
        ...
        self.map = np.zeros((500,500,3), np.uint8) #create a black background for the motion map, by creating a numpy array filled with zeros
        self.map = cv2.circle(self.map, (250, 480), radius=3, color=(238, 130, 238), thickness=-1) #draw a point in the starting point
        qtMap = QtGui.QImage(self.map.data, self.map.shape[1], self.map.shape[0], QtGui.QImage.Format_RGB888).rgbSwapped() #convert the image to be used with qt label
...
#--------------------------------------------------------function to update the motion map-----------------------------------------------------------
    def update_map(self):
        self.map = cv2.circle(self.map, (self.x, self.y), radius=3, color=(238, 130, 238), thickness=-1) #draw a point in the starting point, thickness -1 means a filled circle
        qtMap = QtGui.QImage(self.map.data, self.map.shape[1], self.map.shape[0], QtGui.QImage.Format_RGB888).rgbSwapped() #convert the image to be used with qt label
        pixmap = QtGui.QPixmap.fromImage(qtMap)
        self.motionMapLabel.setPixmap(pixmap)
