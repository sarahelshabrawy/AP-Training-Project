...
class AThread(QThread):
    ...
    def run(self):
        ...
        global arduino
        #print("RUNNING")
        arduinoData = motionUI.motorTypeData + motionUI.motorSpeedData + motionUI.motorDirData + str(diffUI.differences) + str(TrackUI.angle).zfill(3)
        ...
        try : #check if the usb is conneected or not, then try to reconnect
            ...
            if port == "": #if no arduino port is found
                for i in range(len(list(serial.tools.list_ports.comports()))): #look through all the elements of the port list function
                    if "CH340" in list(serial.tools.list_ports.comports()[i])[1]: #if the arduino bootloader is found in the current index
                        port = list(serial.tools.list_ports.comports()[0])[0] #assign the found port to the "port" variable
                        print("\nDevice Connected At",port) #print the found port
                        usbStatus = "Connected"
                        arduino = serial.Serial(port, 9600) #add a serial config where "port" is the com port for arduino and 9600 is the baud rate
                        #time.sleep(1)
